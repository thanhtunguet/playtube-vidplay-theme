...
function copyToClipboard(element) {
  ...
  $temp.remove();
  Snackbar.show({text: 'Link Copied to Clipboard',pos: 'bottom-center',showAction: false});
}
...
function ShortLikeSystem(id, type, self) {
    if (type == 'like') {
        var likes = $(self).attr('data-likes');
        if ($(self).attr('liked')) {
            result = Number(likes) - 1;
        ...
        if ($('.dislike-btn-'+id).attr('data-likes') > 0) {
            if ($('.dislike-btn-'+id).hasClass('active')) {
                $('.dislike-btn-'+id).removeAttr('disliked');
                $('.dislike-btn-'+id).removeClass('active');
                result = Number($('.dislike-btn-'+id).attr('data-likes')) - 1;
                $('.dislike-btn-'+id).find('.short_dislikes').text(numberWithCommas(result));
                $('.dislike-btn-'+id).attr('data-likes', result);
            }
        }
    } else if (type == 'dislike') {
        var dislikes = $(self).attr('data-likes');
        ...
        if ($('.like-btn-'+id).attr('data-likes') > 0) {
            if ($('.like-btn-'+id).hasClass('active')) {
                $('.like-btn-'+id).removeAttr('liked');
                $('.like-btn-'+id).removeClass('active');
                result = Number($('.like-btn-'+id).attr('data-likes')) - 1;
                $('.like-btn-'+id).find('.short_likes').text(numberWithCommas(result));
                $('.like-btn-'+id).attr('data-likes', result);
            }
        }
    }
    $.post(PT_Ajax_Requests_File() + 'aj/like-system/' + type, {id: id, type:type});
...
function OpenCommentsModal(id) {
    ...
    $.post(PT_Ajax_Requests_File() + 'aj/comments/open_modal', {id: id},function (data) {
        if (data.status == 200) {
            $('.shorts_comments_show').html(data.html)
            $('#shorts-pinned-comment').html(data.pinned_comments);
            $('.shorts_loadmore_comments').html($('.shorts_loadmore_comments').attr('data-text'));
            $('.shorts_loadmore_comments').slideDown();
        }
        else{
            $('.shorts_loadmore_comments').slideUp();
            $('#shorts-pinned-comment').html(data.pinned_comments);
            $('.shorts_comments_show').html(data.html)
        }
        setTimeout(function () {
            $('#shorts_comments_modal').modal({
                show: true
            });
        },100);
...
function ShortsComment(button) {
   var text = $('#shorts-comment-textarea').val();
   if (!text) {
   ...
   $.post(PT_Ajax_Requests_File()+'aj/add-comment', {video_id: video_id, text: text}, function(data, textStatus, xhr) {
      if (data.status == 200) {
